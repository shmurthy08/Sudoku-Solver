...
In this literature review, an algorithm analysis was conducted on two Sudoku solver algorithms, which are the pencil-and-paper method and the brute froce method. The pencil-and-paper method is essentially a backtracking algorithm, similar to the backtracking approach used in our human solver algorithm. The brute force method is an algorithm that tries all possible combinations. Based on their tests and the results they gathered, the pencil-and-paper method greatly outperformed the brute force method. As the board increases in diffculty, the number of possible combinations of values also increases. Therfore, as the Sudoku board difficulty increases, so does the brute force algorithms' runtime, since it has a much larger number of possible combinations of values to test. The brute force method and pencil-and-paper methods were pretty close in terms of runtime for easy and medium difficulty boards, however, the brute force method performed exponentially worse for hard and extreme boards.
